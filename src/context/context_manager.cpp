...
Result<ContextBuildResult> ContextManager::build(std::string_view user_message) {
    ...
    if (!reduction_enabled) {
        auto full = render_sections_without_reduction(texts, notes);
        for (const auto& section : kSectionOrder) {
            if (section != "current_request") budgets[section] = utf8_length(full.at(section).raw);
        }
        auto prompt = assemble(full);
        return Result<ContextBuildResult>::success(ContextBuildResult{
            prompt, build_metadata(prompt, full, budgets, JsonValue::Array{}, notes, user_message)});
    }
...
std::map<std::string, SectionRender, std::less<>> ContextManager::render_sections_without_reduction(
    const std::map<std::string, std::string, std::less<>>& texts,
    const std::vector<JsonValue>& notes) const {
    std::map<std::string, SectionRender, std::less<>> result;
    const auto prefix_chars = utf8_length(texts.at("prefix"));
    const auto memory_chars = utf8_length(texts.at("memory"));
    result["prefix"] = {texts.at("prefix"), prefix_chars, texts.at("prefix"), JsonValue::Object{}};
    result["memory"] = {texts.at("memory"), memory_chars, texts.at("memory"), JsonValue::Object{}};
    auto relevant = render_relevant_memory(notes, std::numeric_limits<std::size_t>::max() / 4);
    relevant.budget = utf8_length(relevant.raw);
    relevant.rendered = relevant.raw;
    result["relevant_memory"] = std::move(relevant);
    const auto history = raw_history_text();
    result["history"] = {history, utf8_length(history), history, JsonValue::Object{
        {"collapsed_duplicate_reads", JsonValue(0)}, {"older_entries_count", JsonValue(0)},
        {"rendered_entries", JsonValue::Array{}}, {"reused_file_summary_count", JsonValue(0)},
        {"summarized_tool_count", JsonValue(0)}}};
    result["current_request"] = {texts.at("current_request"), 0, texts.at("current_request"), JsonValue::Object{}};
    return result;
}
